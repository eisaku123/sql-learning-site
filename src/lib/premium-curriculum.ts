...
<span class="sql-keyword">GROUP BY</span> order_id;</code></pre>
    ...
    exercises: [
      ...
      {
        id: "pb-calc-2",
        question: "order_productsテーブルをorder_idでグループ化し、order_idと各order_idごとのSUM(price * quantity)を「合計金額」として取得してください",
      ...
      {
        id: "pb-calc-3",
        question: "productsテーブルをcategoryでグループ化し、カテゴリ名と各カテゴリのAVG(price)を小数点以下2桁に丸めた「平均価格」を取得してください",
      ...
      {
        id: "pb-calc-4",
        question: "ordersテーブルをstatusでグループ化し、statusと各statusのCOUNT(*)を「件数」として取得してください",
...
<span class="sql-keyword">GROUP BY</span> u.id, u.name;</code></pre>
    ...
    exercises: [
      ...
      {
        id: "pb-comp-4",
        question: "users・orders・order_productsを3テーブルJOINし、city・SUM(op.price * op.quantity)を「売上合計」として都市ごとに集計し、売上合計の降順で取得してください（cityがNULLのユーザーは除く）",
      ...
      {
        id: "pb-comp-5",
        question: "productsテーブルとorder_productsをJOINし、カテゴリ名・平均販売単価（AVG(op.price)をROUND小数点以下0桁）・総販売数（SUM(op.quantity)）をカテゴリ別に集計してカテゴリ名のアルファベット順で取得してください",
